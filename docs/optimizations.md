# Agentic Research Factory: Technical Optimizations

Engineering decisions behind scalability, observability, and RAG retrieval, with
the measurements that justify them.

## 1. Scalability
- **Async task orchestration**: agent execution runs on `Celery` with a `Redis` broker and backend.
- **Task delegation**: `routers/runs.py` offloads agent runs to `execute_run_task`, so an API request never blocks on a multi-minute pipeline and workers scale horizontally.

## 2. Observability
- **Metrics**: `prometheus-fastapi-instrumentator` exposes health and throughput at `/metrics`.
- **Cost tracking**: the `RunCost` model records token usage and cost per agent per run, which is what `/analytics/costs` reads.
...
- **Why page by page**: the page number *is* the citation. A whole-document `export_to_markdown()` flattens away Docling's provenance, leaving every chunk with `page: None` and every internal citation reading "Page: N/A". Exporting one page at a time preserves the Markdown structure the splitter needs and gives each chunk a real page.
...
- **Hybrid Search**: Dual indexing in `rag.py` — `HNSW` (via `vecs`) for semantic vector similarity, plus a GIN index over `to_tsvector('english', metadata->>'text')` for keyword retrieval. `vecs` only exposes vector indexes, so the lexical half is PostgreSQL full-text search ranked by `ts_rank_cd` rather than BM25; both halves run per sub-query and merge into one deduplicated candidate pool.
  ...
  The lexical half matches **any** query term, not all of them, via `websearch_to_tsquery` over `or`-joined terms — which keeps hyphenated identifiers as phrases, while `ts_rank_cd` still ranks chunks matching more of the query higher. `plainto_tsquery` ANDs every lexeme instead: that suits a bag of keywords and fails on the question-shaped strings agents actually send, since `"What does ACM-429 mean?"` becomes `'acm' & '-429' & 'mean'` and the one chunk documenting that error code does not match unless it also says "mean". The failure is silent — no error anywhere, just a candidate pool sitting at exactly the vector half's limit for every query, which is the symptom the retrieval eval watches for (`test_hybrid_retrieval_contributes_more_than_the_vector_half` asserts `mean_pool_size > _CANDIDATES_PER_QUERY`).
...
### Evaluation
Three harnesses, measuring different things. The gap between the pair-level and pipeline-level numbers is what set the abstention threshold — see below.
...
**The threshold must come from the pipeline, not from the calibration.** The gate never sees a passage in isolation — it sees the best chunk retrieval could find anywhere in the corpus, and best-of-pool scores run far above isolated-pair scores. Calibration puts off-topic *pairs* in a tight band near −11.2, which makes −11.0 look like the natural cut; against the real pipeline −11.0 rejects only **1 of 10** unanswerable queries. `RAG_MIN_RERANK_SCORE = -9.0` rejects 7/10 at no measured cost in false abstentions (1/62 at either threshold, and that one is a colloquial paraphrase −11.0 also missed). It is deliberately not pushed to −8.5, where the numbers look better still: the lowest answerable query that passes scores −8.351, and a threshold 0.15 away from a real answer is tuned to this corpus rather than robust to the next one. `test_pair_level_threshold_would_barely_gate_anything` pins this.

**Known limits.** 54 chunks is small enough that the vector half alone achieves pool recall 1.000, so this corpus cannot demonstrate the lexical half's contribution to recall — it can only show its cost, and it does: a live lexical half widens the mean pool from 10.0 to 13.4 and moves hit@5 from 0.968 down to 0.903, because the extra candidates are distractors the re-ranker sometimes prefers. (`dense+rerank` in the ablation scores exactly 0.968 — the same number production measured while the lexical half was silently ANDing itself into nothing.) That trade should invert at production corpus sizes, where vector recall stops being perfect, but this harness cannot show it.
...
### Retrieval Scoping
- **Vertical metadata filter**: `ingest_documents` tags each chunk with an optional `vertical`; `search_documents` takes an optional `vertical_filter` and applies it as a `query_filter` at the database level, so an unrelated subset is never scored in the first place.

---
*Source of truth for everything above: `backend/tools/rag.py`, `backend/services/pdf_service.py`, `backend/services/query_rewriter.py`, `backend/evals/`.*
